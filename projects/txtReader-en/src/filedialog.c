...
int fileMenuSelect(file_info_t files[], int max_n, char *pathname, char *filter) {
    ...
    drawableitems = max_n;
    if (max_n)
        sprintf(buf,"OPEN %s [%d/%d]",filter,listindex+menuindex+1,max_n);
    else
        sprintf(buf,"OPEN %s [--/--]",filter);
    ...
    if (max_n == 0) {
        ProcessPrintChars(936);
        PrintCXY(LCD_WIDTH_PX/2-60,LCD_HEIGHT_PX/2-24,"No Files",0,-1,COLOR_BLACK,COLOR_WHITE,1,0);
        ProcessPrintChars(0);
    }
